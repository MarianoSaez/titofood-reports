...
type RestOf<T extends IReporte | NewReporte> = Omit<T, 'fechaInicio' | 'fechaFin'> & {
...
export class ReporteService {
  ...
  protected convertDateFromClient<T extends IReporte | NewReporte | PartialUpdateReporte>(reporte: T): RestOf<T> {
    return {
      ...reporte,
      fechaInicio: reporte.fechaInicio?.toJSON() ?? null,
      fechaFin: reporte.fechaFin?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReporte: RestReporte): IReporte {
    return {
      ...restReporte,
      fechaInicio: restReporte.fechaInicio ? dayjs(restReporte.fechaInicio) : undefined,
      fechaFin: restReporte.fechaFin ? dayjs(restReporte.fechaFin) : undefined,
    };
  }
